...
public class GroceryStore {
	...
	public static void addWeightItemMenu() {
		...
		for(Item x : cart.getItems()){
			if(x.getName().equals(name)){
				WeightItem nw = (WeightItem) x;
				nw.setWeight(nw.getWeight() + nw.scale());
				return;
			};
		}

		unitPrice = in.nextDouble();
		cart.getItems().add(new WeightItem(name,unitPrice));
	}
	...
	public static void addUnitItemMenu() {
		...
		for(Item x : cart.getItems()){
			if(x.getName().equals(name)){
				UnitItem nw = (UnitItem) x;
				nw.setAmount(nw.getAmount() + amount);
				return;
			};
		}
		unitPrice = in.nextDouble();
		cart.getItems().add(new UnitItem(name,unitPrice,amount));
	}
	...
	public static void viewAllItemMenu() {
		...
		int i=1;
		for (Item x : cart.getItems()) {
			System.out.println("#"+i+"\t"+x.getClass()+" : "+x.getName()+"\t"+x.getUnitPrice()+" "+x.cost());
		}
	}
	...
	public static void checkOutMenu(){
		...
		//fill code
		double tt = cart.checkOut(),dis=0;
		if(cart.isDiscounted()){
			System.out.println("Congratulation! You got a "+cart.getDiscount()+"% discount");
			dis = tt * cart.getDiscount()/100;
			tt -= dis;
		}
		System.out.println("Grand total cost : $"+tt+" (Save $"+dis+")");
	}
