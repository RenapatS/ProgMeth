...
public class CPTSMachine {
	...
	public static boolean isStationExisted(String name) {
		...
		for(int i=0;i<stationlist.size();i++){
			if(name.equals(stationlist.get(i).getName())) return true;
		} return false;
	}
	...
	public static boolean buyTicket(int type, Station start, Station end) {
		...
		if (start == null || end == null) {
			System.out.println("This ticket cannot be bought."); // หรือ message ตามที่โจทย์ต้องการ
			return false;
		}
		Ticket t = new Ticket(type, start, end);
		if(/* CONDITION */isStationExisted(start.getName()) && isStationExisted(end.getName())
				&& type >=0 && type<3 &&(type!=2 || t.getStationDistance(start,end)<=6)
				&& t.isStationValid(start, end)) {

			System.out.println("Bought "+ start.getName() +" to "+end.getName()+/* FILL CODE */", for "+ /* FILL CODE */t.calculatePrice()+ " Baht!");
			ticketlist.add(t);
			return true;
		}else {
